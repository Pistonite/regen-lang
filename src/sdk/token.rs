...
pub enum TokenizerAction<T> {
    ...
    Keep(bool /* should extract */, Token<T>),
...
impl<'t, T> TokenStream<'t, T> {
    ...
    pub fn new(tokens:&'t[Token<T>], max_stack_size: usize) -> Self {
        Self {
            tokens,
            index: 0,
            stack: Vec::new(),
    ...
    pub fn get_guess_err_token(&self) -> Option<&'t Token<T>> {
    ...
    pub fn consume(&mut self) -> Option<&'t Token<T>> {
