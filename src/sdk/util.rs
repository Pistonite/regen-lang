...
impl Error {
    ...
    pub fn from_token<T>(token: &Token<T>, msg: String, help: String) -> Self {
        Self {
            msg,
            help: Some(help),
            pos: token.pos,
    ...
    pub fn from_token_without_help<T>(token: &Token<T>, msg: String) -> Self {
        Self {
            msg,
            help: None,
            pos: token.pos,
    ...
    pub fn pretty(&self, source: &str, context: usize) -> Result<String, std::fmt::Error> {
        ...
        for (i, line_str) in source.lines().skip(start-1).take(context*2+1).enumerate() {
            ...
            let help = self.help.as_ref().map(|s| format!(" help: {}", s)).unwrap_or_default();
            if current == l {
                writeln!(output, "{:3} | {:>width$}{t}{help}", "", "", width = c-1, t= "^".repeat(self.pos.1 - self.pos.0))?;
            }
        }
