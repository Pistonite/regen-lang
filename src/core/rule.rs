    ...
    fn resolve_func_concat_ret_type<'s>(
        ...
        if let ParamType::Item(_, last_name) = last_var_type {
            ...
            ret_types.insert(
                name.clone(),
                RetType::Unresolved(
                    "Return type of the last variable in a concatenation must be a vector"
                        .to_owned(),
                ),
            );
            return;
...
pub fn parse_rule(
    ...
    if let RuleValue::Union(union) = &value {
        ...
        if union.contains(&pt.m_rule_name) {
            ...
            errors.push(Error::from_token(&pt.ast.m_rule_name_2, msg, help));
        }
    }
...
fn parse_rule_value_function(
    ...
    let body = match pt.m_body.as_mut().as_mut() {
        None => {
            ...
            errors.push(Error::from_token(&pt.ast.m___4 /*;*/, msg, help));
            None
        }
        Some(body) => {
