...
impl Language {
  pub fn from_pt_ctx(pt: Vec<pt::TopLevelStatement>, ctx: &Context) -> Result<Self, Vec<Error>> {
...
fn validate_references(pt: &[pt::TopLevelStatement]) -> Result<() /* semantics */, Vec<Error>> {
  ...
  for pt in pt {
    match pt.m_body.as_ref() {
      ...
      pt::TopLevelDefine::DefineRuleStatement(rule) => {
      ...
      // the others don't define names, but written out explicitly so it gives an error if we add another top level statement
      pt::TopLevelDefine::TokenLiteral(_) => {}
      pt::TopLevelDefine::DefineTokenRuleStatement(_) => {}
...
fn extract_from_pt(
  ...
  for pt in pt {
    match *pt.m_body {
      ...
      pt::TopLevelDefine::DefineRuleStatement(mut rule) => {
      ...
      // context is extracted from a context (just for the sake of it)
      pt::TopLevelDefine::DefineContextStatement(_) => {}
    }
  }
