      ...
      match &param.param_type.decor {
        ...
        ParamDecorType::Vec(optional) => {
          if *optional {
            ...
            t = format!("{{ let mut v = vec![required!(ts, {t})?]; list!(ts, v, {t}) }}");
          }
        }
      }
      ...
      let code = match &param.param_type.data {
        ...
        _ => {
          ...
          let inner = match &param.param_type.decor {
            ...
            ParamDecorType::Optional => {
              format!("if let Some(m) = &self.{member} {{ si.set(m, {semantic_expr}); }}")
            }
            ParamDecorType::Vec(_) => {
        ...
        let l = match &param.param_type.data {
          ...
          ParamDataType::Flag(_) => {
            match &param.param_type.decor {
              ...
              ParamDecorType::Optional => Some(codeln!(f "{member}: ast.{member}.is_some(),")),
              ParamDecorType::Vec(_) => Some(codeln!(f "{member}: ast.{member}.len() as usize,")),
            }
          }
        };
        if let Some(code) = l {
