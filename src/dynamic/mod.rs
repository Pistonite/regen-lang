...
impl DynAstNode {
      ...
      RuleValue::Function(params) => {
        ...
        for param in params {
          ...
          match &param.param_type.data {
            ParamDataType::Rule => {
              impl_dyn_ast_parse_param!(param, ts, children, Self::parse_rule(ts, lang, type_name))
            }
            ParamDataType::String => {
              impl_dyn_ast_parse_param!(param, ts, children, Self::parse_token(ts, type_name))
            }
  ...
  fn apply_semantic(&self, ctx: &mut DynCtx, ovr: &Option<DynTok>) {
    match self {
      ...
      DynAstNode::Interior(rule_name, children) | DynAstNode::InteriorVec(rule_name, children) => {
